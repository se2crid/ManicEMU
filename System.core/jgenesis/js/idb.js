...
/**
 * 写入原始数据（用于 save state）
 * @param key {string}
 * @param bytes {Uint8Array}
 * @returns {Promise<void>}
 */
export function writeRawData(key, bytes) {
    return new Promise((resolve, reject) => {
        const openReq = indexedDB.open("save_states");

        openReq.onerror = () => reject(`Failed to open IndexedDB for write, key ${key}`);
        ...
        openReq.onsuccess = (event) => {
            ...
            const objectStore = tx.objectStore("states");
            const putReq = objectStore.put({ key, data: bytes, timestamp: Date.now() });
